#!/usr/bin/env node
// Fails on a PRECACHE entry in main-site/sw.js with no file on disk, and on
// a module, stylesheet or the word list existing but not being precached,
// since either one is a hole in offline use.
//
...
// Practice rounds, the offline game, need these.
for (const route of ["/", "/wordlist.json", "/manifest.json"]) {
  if (!seen.has(route)) unlisted.push(route);
}
